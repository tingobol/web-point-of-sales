...
class Penjualan extends CI_Controller{
	...
	function get_detail_barang()
	{
		...
		$data=$this->Admin_model->show_list("inv_barang_view","where Nama_Barang='".$nm_br."'");
	...
	function set_detail_trans(){
		...
		$data['ID_Barang']	=rdb('inv_barang','ID','ID',"where Nama_Barang='".$nm_barang."'");
		$data['ID_Jenis']	=$_POST['cbayar'];
...
*/			}
	...
	*/
	...
	function get_list_trans()
	{
	    ...
	    $where.=empty($_POST['cari'])?'':" and Nama like '".$_POST['cari']."%'";
		...
		foreach($data as $r)
		{
		 ...
		 echo tr('xx list_genap tree_'.$r->ID_j).td($n,'center').
			...
			td().
			td().
			td(($cek!='' && $otok!='')?img_aksi($r->ID_j.':'.tglFromSql($r->Tanggal),false,'edit'):img_aksi($r->ID_j,false,'delete'),'center').
		   _tr();
			  ...
			  foreach($datax as $d)
			  {
				 ...
				 echo tr().td($x.nbs(5),'right').
					  td(strtoupper(rdb("inv_barang","kode","kode","where ID='".$d->ID_Barang."'")),'center').
					  td(rdb("inv_barang","Nama_Barang","Nama_Barang","where ID='".$d->ID_Barang."'"),'left\' colspan=\'2').
					  td($d->Jumlah,'center').
					  td(number_format($d->Harga,2),'right').
					  td(number_format(($d->Jumlah*$d->Harga),2),'right').
					  td(($otok!='' && $r->ID_Post==0)?img_aksi($r->ID,true):'','center').
					 _tr();
					 $total +=$d->Jumlah;
					 $h_tot +=$d->Harga;
					 $s_tot +=($d->Jumlah * $d->Harga);
			  }
